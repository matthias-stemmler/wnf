...
fn set_with_expected_change_stamp() {
    ...
    let updated = state.update(&0x11111111, WnfChangeStamp::initial()).unwrap();
    assert!(updated);
    assert_eq!(state.get().unwrap(), 0x11111111);

    let updated = state.update(&0x22222222, WnfChangeStamp::from(1)).unwrap();
    assert!(updated);
    assert_eq!(state.get().unwrap(), 0x22222222);

    let updated = state.update(&0x33333333, WnfChangeStamp::from(1)).unwrap();
    assert!(!updated);
    assert_eq!(state.get().unwrap(), 0x22222222);

    let updated = state.update(&0x44444444, WnfChangeStamp::from(3)).unwrap();
    assert!(!updated);
    assert_eq!(state.get().unwrap(), 0x22222222);
}
