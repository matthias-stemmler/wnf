//! Raw bindings to some of the WNF routines of the Windows Native API (NTAPI)
//!
...
///
/// Each of the functions in this module has a counterpart whose name starts with `Zw` instead of `Nt`. For calls from
/// user mode (which is the only mode supported by this crate), these two classes of functions behave identically.
///
...
mod ntexapi {
    ...
    extern "system" {
        ...
        /// - (in, out) `buffer_size`: Pointer to a `u32` buffer containing the size of the buffer pointed to by
        ///   `buffer` in bytes and receiving the actual required buffer size in bytes
        ///
        /// # Returns
        /// An `NTSTATUS` value that is `>= 0` on success and `< 0` on failure
        ///
        ...
        ///
        /// The information is written into a 4-byte buffer and is always a boolean value, where `0` means `false` and
        /// `1` means `true`.
        ///
        /// # Arguments
        /// - (in) `state_name`: Pointer to the state name
        /// - (in) `name_info_class`: Tag of the class of information to obtain; at least the following values are
        ///   valid:
        ///   - `0`: "State name exist"
        ...
        /// - (in) `type_id`: Pointer to a GUID used as the type ID, can be a null pointer
        /// - (in) `explicit_scope`: Irrelevant, can be a null pointer
        /// - (in) `matching_change_stamp`: The expected current change stamp of the state (only relevant if
        ///   `check_stamp` is `1`)
        ...
        /// An `NTSTATUS` value that is `>= 0` on success and `< 0` on failure
        ///
        /// In particular, returns [`STATUS_UNSUCCESSFUL`](windows::Win32::Foundation::STATUS_UNSUCCESSFUL) if the
        /// update was not performed because `check_stamp` was `1` and the current change stamp was different
        /// from `matching_change_stamp`.
        ///
        /// # Safety
        /// - `state_name` must point to a valid `u64`
...
mod ntrtl {
    ...
    extern "system" {
        ...
        /// - `context` is the `callback_context` passed to some successful call to
        ...
        ///
        /// # Assumptions
        /// - If `subscription_handle` was returned from a successful call of `RtlSubscribeWnfStateChangeNotification(_,
        ///   _, _, callback, callback_context, _, _, _)`, where `callback_context` is unique among all such calls, and
        ///   this function succeeds, then `callback` is not called with `callback_context` anymore.
