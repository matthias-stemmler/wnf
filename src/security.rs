...
use windows::Win32::{
    Foundation::PSID,
    Security::{
        self, CreateWellKnownSid, WinWorldSid, ACCESS_ALLOWED_ACE, ACL, ACL_REVISION,
        PSECURITY_DESCRIPTOR, SECURITY_DESCRIPTOR, SID,
    },
...
impl SecurityDescriptor {
    ...
    pub(crate) fn create_everyone_generic_all() -> Result<Self, SecurityCreateError> {
        ...
        let acl_size =
            mem::size_of::<ACL>() + mem::size_of::<ACCESS_ALLOWED_ACE>() + sid_size as usize
                - mem::size_of::<u32>();

        let acl_layout =
            unsafe { Layout::from_size_align_unchecked(acl_size, mem::align_of::<u32>()) };

        let acl_buffer = unsafe { alloc::alloc(acl_layout) };

        unsafe { Security::InitializeAcl(acl_buffer.cast(), acl_size as u32, ACL_REVISION.0) }
            .ok()?;

        unsafe {
            Security::AddAccessAllowedAce(acl_buffer.cast(), ACL_REVISION.0, GENERIC_ALL, psid)
        }
        .ok()?;

        let mut raw_security_descriptor = SECURITY_DESCRIPTOR::default();
        let psecurity_descriptor = PSECURITY_DESCRIPTOR(
            &mut raw_security_descriptor as *mut SECURITY_DESCRIPTOR as *mut c_void,
        );
